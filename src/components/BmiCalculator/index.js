...
const BmiCalculator = () => {
  const sHeight = JSON.parse(localStorage.getItem('Height'))
  const sWeight = JSON.parse(localStorage.getItem('weight'))
  const [height, setHeight] = useState(sHeight !== null ? sHeight : 170)
  const [weight, setWeight] = useState(sWeight !== null ? sWeight : 60)
